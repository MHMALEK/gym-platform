...
import { getClient, updateClient, type ClientRow } from "@/lib/clients";
...
export default function ClientEditScreen() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const c: ClientRow = await getClient(id);
      setName(c.name ?? "");
      setEmail(c.email ?? "");
      setPhone(c.phone ?? "");
      setNotes(c.notes ?? "");
      setStatus(c.status ?? "active");
      setAccountStatus(c.account_status ?? "good_standing");
    } catch (e) {
  ...
  const save = async () => {
    ...
    try {
      ...
      router.replace(`/(tabs)/clients/${id}`);
    } catch (e) {
